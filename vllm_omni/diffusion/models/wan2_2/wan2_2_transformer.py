...
class WanFeedForward(nn.Module):
    ...
    def __init__(
        ...
        self.net_0 = ColumnParallelGELU(dim, inner_dim, approximate="tanh", bias=bias, quant_config=quant_config,
                                         prefix=f"{prefix}.net_0" if prefix else "")
        ...
        self.net_2 = RowParallelLinear(
            ...
            return_bias=False,
            quant_config=quant_config,
            prefix=f"{prefix}.net_2" if prefix else "",
        )
...
class WanSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        self.to_qkv = QKVParallelLinear(
            ...
            bias=True,
            quant_config=quant_config,
            prefix=f"{prefix}.to_qkv" if prefix else "",
        )
    ...
    def __init__(
        ...
        if added_kv_proj_dim is not None:
            self.add_k_proj = ColumnParallelLinear(
                ...
                return_bias=False,
                quant_config=quant_config,
                prefix=f"{prefix}.add_k_proj" if prefix else "",
            )
            self.add_v_proj = ColumnParallelLinear(
                ...
                return_bias=False,
                quant_config=quant_config,
                prefix=f"{prefix}.add_v_proj" if prefix else "",
            )
            if get_tensor_model_parallel_world_size() > 1:
...
class WanTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn1 = WanSelfAttention(
            ...
            eps=eps,
            quant_config=quant_config,
            prefix=f"{prefix}.attn1" if prefix else "",
        )
        ...
        self.attn2 = WanCrossAttention(
            ...
            quant_config=quant_config,
            prefix=f"{prefix}.attn2" if prefix else "",
        )
...
class WanTransformer3DModel(nn.Module):
    ...
    def __init__(
        ...
        # prefix="blocks.{i}" follows the standard vLLM convention: it is
        # threaded down to every LinearBase so that quant_config.get_quant_method()
        # receives the full dotted path (e.g. "blocks.5.attn1.to_qkv").
        # DiffusionMixedMXFPConfig uses this to dispatch MXFP8 vs MXFP4 by
        # block index; other configs ignore it.
